...
class Player:
    ...
    def control(self, keys, fly):
        ...
        if fly:  # if flying boolean is true
            ...
            if keys[self.controls[0]] != keys[self.controls[1]]:  # if only one button in the horizontal axis is being hit
            ...
            if keys[self.controls[0]] != keys[self.controls[1]]:  # if player is either moving left XOR right
                if keys[self.controls[0]] and (abs(self.vx) < self.max_vx or self.vx > 0):  # if player is hitting left and is moving left
                    self.dir = -1  # player is moving left
                    self.flipped = 1  # player is facing left (flipped)
                    self.pointing = -1  # player is pointing left
                if keys[self.controls[1]] and (self.vx < self.max_vx or self.vx < 0):  # if player is hitting right and is moving right
            ...
            if self.dir:  # if player has a direction other than 0
