...
class Player(object):
    ...
    def __init__(self, PlayerNumber, PlayerUsername):
        ...
        self.x, self.y, self.spawnx, self.spawny, self.inventory, self.health, self.hunger, self.satura = self.get_playerInfo()
    ...
    def get_playerInfo(self):
        try:
            ...
            PlayerData[self.UUID] = [(), [[0] * 2] * 36, 10, 10, 10]
            return PlayerData[self.UUID]

    def changeLocation(self, cord):
...
if __name__ == '__main__':
    ...
    reciever = Process(target=sendAllPlayer, args=(messageQueue, server))
    reciever.start()
    ...
    while True:
        ...
        if command == 0:
            ...
            players[address] = (Player(PN, message[1]), message[2])

        elif command == 1:
            # Player movement
            # Data: [1, <cordx>, <cordy>]
            players[address][1].changelocation((message[1], message[2]))
